# Puxa uma copia FRESCA do banco de producao (VPS) para o Postgres local (Docker).
# Sentido unico: producao -> local. Nunca escreve em producao.
#
# Pre-requisitos:
...
#   .\scripts\db-pull.ps1 -SchemaOnly           # so estrutura, sem dados
#   .\scripts\db-pull.ps1 -VpsUser ubuntu       # outro usuario SSH
...
# Carrega a DATABASE_URL do .env.local da VPS e roda o pg_dump la dentro.
...
Write-Host "OK! Banco local sincronizado com a producao." -ForegroundColor Green
