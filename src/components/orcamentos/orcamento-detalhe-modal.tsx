      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                {orcamento.pedido_convertido?.codigo && (
                  <span className="text-xs" style={{ color: 'var(--ac-muted)' }}>
                    Pedido <strong style={{ color: 'var(--ac-text)' }}>{orcamento.pedido_convertido.codigo}</strong>
            ...
            <p className="text-sm" style={{ color: 'var(--ac-muted)' }}>
              <strong style={{ color: 'var(--ac-text)' }}>{orcamento.cliente?.nome ?? 'Sem cliente'}</strong>
              {orcamento.cliente?.tipo && (
                <span className="ml-2 text-xs" style={{ color: 'var(--ac-muted)' }}>({orcamento.cliente.tipo})</span>
              )}
            </p>
            ...
            {orcamento.vendedor?.nome && (
              <p className="text-xs" style={{ color: 'var(--ac-muted)' }}>
                Vendedor: <strong style={{ color: 'var(--ac-text)' }}>{orcamento.vendedor.nome}</strong>
              </p>
            )}
            {orcamento.observacao && (
              <p className="text-xs mt-0.5 italic" style={{ color: 'var(--ac-muted)' }}>{orcamento.observacao}</p>
            )}
          </div>
                    ...
                    <td className="px-4 py-2.5" style={{ color: 'var(--ac-text)' }}>
                      <div className="flex items-center gap-3">
                        {thumbUrl ? (
                          ...
                          >
                            <img src="/images/favicon-yellow.png" alt="Sem foto" width={18} height={18} style={{ objectFit: 'contain' }} />
                          </div>
                        )}
        ...
        {confirmandoConversao && (
          <div
            ...
            style={{ background: 'color-mix(in srgb, var(--ac-accent) 8%, transparent)', border: '1px solid var(--ac-border)' }}
          >
            <div>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="secondary" onClick={() => setConfirmandoConversao(false)} disabled={loading === 'converter'}>
                Cancelar
              </Button>
